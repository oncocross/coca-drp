...
class GCNReLU(nn.Module):
    def __init__(self, in_feats, hidden_feats,activation=None):
        ...
        self.gnn = GCN(in_feats,hidden_feats,gnn_norm=None,activation=None,residual=None,batchnorm=None,dropout=None)
...
class GCADTI(nn.Module):
    ...
    def forward(self, prot_x, drug_x):
        prot_x = self.prot_embed(prot_x).permute(0, 2, 1) # (B, L) → (B, L, 128) # (B, 128, L)
        prot_x = self.prot_conv1(prot_x) # (B, 128, L) → (B, 128, L-2)
        prot_x = self.bn1(prot_x)
        prot_x = self.prot_conv2(prot_x) # → (B, 128, L-2-5) = (B, 128, L-7)
        prot_x = self.bn2(prot_x)
        prot_x = self.prot_conv3(prot_x) # → (B, 128, L-7-8) = (B, 128, L-15)
        prot_x = self.bn3(prot_x)
        drug_x = self.drug_gcn(drug_x)  # → (B, N, 128)
        prot_x_g = self.prot_mut_att(drug_x.permute(0, 2, 1), prot_x) # (B, 128, N) , (B, 128, L′) -> (B, 128, L′)
        drug_x_g = self.drug_mut_att(prot_x, drug_x.permute(0, 2, 1))# (B, 128, L′), (B, 128, N) ,  -> (B, 128, N)
        prot_x = self.prot_pool(prot_x_g).squeeze(-1) #(B, 128)
        drug_x = self.drug_pool(drug_x_g).squeeze(-1) #(B, 128)
        x_cat = torch.cat([prot_x, drug_x], dim=-1) #(B, 256)
