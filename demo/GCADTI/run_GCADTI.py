...
from GCADTI.DTIDataset import *
from GCADTI.GCADTI import *
...
if __name__ == '__main__':
    ...
    if args.task == 'training':
        ...
        # pred_dti=pd.DataFrame(pred_dti)
        # pred_dti.columns=['smiles','sequence']
        # pred_dti = pred_dti[pred_dti['smiles'] == pred_dti.iloc[0,0]]
        # pred_dti['label']=0 
        # with torch.no_grad():
        #     pred_set=DTIDataset(pred_dti)
        #     y_label, y_pred  = net.predict(pred_set)
        ...
        #     for j in prot:
        #         pred_dti.append([i,j])
        # pred_dti=pd.DataFrame(pred_dti).iloc[:3,:]
        # pred_dti.columns=['smiles','sequence']
        # pred_dti = pred_dti[pred_dti['smiles'] == pred_dti.iloc[0,0]]
        # pred_dti['label']=0 
        # with torch.no_grad():
        ...
        # IC_test=pd.read_csv('./IC_test.csv',index_col=0)
        # with torch.no_grad():
        #     pred_set=DTIDataset(IC_test)
        #     y_label, y_pred  = net.predict(pred_set)
        # IC_test['y_label']=y_label
        # IC_test['y_pred']=y_pred 
        # IC_test.to_csv('IC_test_pred.csv')
        
        # meta_var = pd.read_pickle('./meta_var.pkl')
        # smiles_dic = meta_var['pubchemid2smiles']
        ...
        # pred_dti['label']=y_pred
        # pred_dti['cid']=cids
        # pred_dti.to_csv('pred_dti_gdsc_asg.csv')
        
        # with open('dffndds_drug_set.json', 'r', encoding='utf-8') as f:
        #     data = json.load(f)
            
        # smiles_dic = {}
        # for d in data:
        #     smiles_dic[d] = data[d]['smiles']
        ...
        # pred_dti['label']=y_pred
        # pred_dti['cid']=cids
        # pred_dti.to_csv('pred_dti_dffndds_asg_v2.csv')
        # import pandas as pd
        # data = pd.read_csv('train.csv')
            
        # smiles_dic = dict(zip(data['ID'], data['Canonical_Smiles']))
        ...
        # pred_dti['label']=y_pred
        # pred_dti['ID']=cids
        # pred_dti.to_csv('pred_dti_dacon.csv')
        import pickle
        with open("cid2smiles.pkl", "rb") as f:
        ...
        pred_dti['label']=y_pred
        pred_dti['ID']=cids
        pred_dti.to_csv('pred_dti_procan.csv')
